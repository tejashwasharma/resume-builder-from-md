...
propose one for six services. See [microservices](../03-backend/05-microservices.md).
...
signal. [Consensus and coordination](25-consensus-coordination.md) has the
argument in full.
...
| Talking between services | `@grpc/grpc-js` | Typed contracts and binary framing for internal calls; see [api styles](../03-backend/04-api-styles.md) |
...
[resilience and rate limiting](29-resilience-rate-limiting.md), and you learn
...
The map above is drawn with open-source names, and it is worth knowing that
most of them are re-implementations of systems Google published first. Nginx
and Envoy at the edge play the role of Google's front-end proxies; Kubernetes
*is* Borg's ideas in the open (*Large-scale cluster management at Google with
Borg*, EuroSys 2015); etcd is a Raft implementation of the role Chubby plays
inside Google (*The Chubby lock service for loosely-coupled distributed
systems*, Burrows, OSDI 2006 — a Paxos-replicated store used for locks,
leader election and small config, deliberately kept small and slow-changing);
Cassandra and HBase descend from *Bigtable* (Chang et al., OSDI 2006);
Prometheus's data model was designed by ex-Google engineers after Borgmon,
the internal monitoring system described in the SRE book (*Site Reliability
Engineering*, O'Reilly 2016, chapter 10).

Why this matters in the room: when you say "I'd put etcd here for leader
election", a Google interviewer hears "Chubby", and knows the follow-ups —
what happens when the coordination service is unavailable, why you keep it
small, why you never put request-path data in it. Naming the lineage shows you
understand the *role* the tool plays, not just its name.

**The design-round question this chapter answers:** *"Which of these boxes
would you pick, and why not the other one?"* The interviewer is not testing
brand knowledge; they are testing whether you can say what problem each layer
solves and what it costs. "Kafka here, because I need replay and multiple
consumers; SQS would be simpler but loses ordering and replay" is the shape of
a passing answer.
