...
> handle the user's next request. See [caching at scale](28-caching-at-scale.md).
...
chapter [22](22-failure-modes-and-fallacies.md) takes this apart properly.
...
[chapter 23](23-consistency-cap-pacelc.md).
...
Each box is a chapter. [Building blocks](35-building-blocks.md)
in Part 03 covers when to reach for each; the specific technologies that fill
each box are in [the technology landscape](21-the-technology-landscape.md).
...
| --- | --- | --- |
| 1 | [Foundations and failure modes](22-failure-modes-and-fallacies.md) | Partial failure, properly. Everything else depends on it |
| 2 | [CAP, PACELC and consistency](23-consistency-cap-pacelc.md) | The trade you just met, formalised |
| 3 | [Replication and partitioning](24-replication-partitioning.md) | How data survives a machine dying, and how it outgrows one machine |
| 4 | [Caching at scale](28-caching-at-scale.md) | The highest-leverage lever, and its failure modes |
| 5 | [Messaging and streams](27-messaging-streams.md) | Queues and logs — how services stop calling each other directly |
| 6 | [Idempotency and transactions](26-idempotency-transactions.md) | Making retries safe; work spanning services |
| 7 | [Resilience and rate limiting](29-resilience-rate-limiting.md) | Stopping one failure becoming all of them |
| 8 | [Consensus and coordination](25-consensus-coordination.md) | The hardest, and the least often asked. Last on purpose |
...
picking *systems* (see [the technology landscape](21-the-technology-landscape.md)).
...
Every idea in this chapter has a Google-published system behind it, and the
design round expects you to know that the ideas are not academic. The
canonical starting point is *The Google File System* (Ghemawat, Gobioff,
Leung — SOSP 2003): thousands of commodity machines, files chunked and
replicated three ways, one master holding metadata, and the explicit
assumption that **component failure is the norm, not the exception**. That
one sentence from the paper is the whole reason this chapter exists. Its
successor, Colossus (described publicly in a 2021 Google Cloud blog post,
"Colossus under the hood"), keeps the shape — metadata separated from data,
replication and erasure coding across many disks — and underpins Bigtable,
Spanner and Cloud Storage.

Compute sits on the same premise. *Large-scale cluster management at Google
with Borg* (Verma et al., EuroSys 2015) describes scheduling hundreds of
thousands of jobs across cells of tens of thousands of machines, restarting
tasks on failed machines automatically. Kubernetes is its open-source
descendant, and "stateless replicas behind a scheduler that reschedules on
failure" — the scale-out story told above — is exactly Borg's model.

**The design-round question this chapter answers:** *"Why not one big
machine?"* and, more sharply, *"What is your system's single point of
failure, and what happens when it dies?"* Answer with the GFS premise: the
design starts from the assumption that any machine can vanish, and every
component is either replicated or restartable. If you can say which of your
boxes is neither, you have found the part of your design the interviewer
will push on next.
