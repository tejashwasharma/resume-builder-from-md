...
## Follow-ups Google would ask

The auth-service design above is the one on your resume, so a Google
interviewer will push past it. These are the four pushes to have rehearsed.

**"Make it multi-region, active-active."** The hard part is not the stateless
token-validation path — that replicates trivially with the public keys. The
hard part is the writes that must be globally ordered: session creation,
revocation and key rotation. Two honest options: (a) one home region per
tenant for writes, with reads everywhere and a routing layer that knows the
home — simple, and a tenant's latency to its home is the cost; (b) a
strongly consistent multi-region store (the Spanner model from
[23](23-consistency-cap-pacelc.md)) for the small revocation and key tables
only, with everything else regional. Say which you would pick and why the
revocation table is the one that needs consistency: a revoked session that is
still valid in another region is the "new enemy" problem from
[38](38-design-rbac-sso-sessions.md).

**"A session is revoked. How fast is it dead everywhere?"** Three tiers, and
the answer should name all three: short-lived access tokens bound the
staleness (say 5 minutes); a revocation list published to every validator via
pub/sub or a replicated cache shrinks it to seconds; and for the cases that
must be immediate (admin lockout, credential compromise) the validator checks
a strongly consistent store on the request path and you pay the latency. The
senior move is to say which tier applies to which event.

**"Rotate the signing key with zero downtime."** Publish the new public key
in the JWKS *before* signing anything with it; overlap both keys for at least
one access-token lifetime; sign with the new key; retire the old one after
the last token it signed has expired. The `kid` header is what makes the
overlap work. Say what happens if a validator caches the JWKS too long, and
what the cache TTL therefore must be relative to the rotation schedule.

**"Now 10x."** Validation scales linearly (stateless, public-key check).
The bottlenecks are the login path (the password hash is CPU-bound by design,
so rate-limit and shard by user), the session store (partition by user id;
the hot key is the one tenant with a bulk import), and the audit log (append
only, batched, shipped to cold storage — never on the request path). Then
say which of those you would fix first, and what you would measure to know
you had to.
