...
The public reference is Google Cloud **Pub/Sub**, whose documentation states
the semantics this chapter is built on: delivery is **at least once** by
default (with an exactly-once option for a single region that still depends
on subscriber acknowledgement), unacknowledged messages are redelivered, and
ordering is only guaranteed for messages that share an **ordering key** and
are published from the same region. Those three sentences contain the whole
consumer-side discipline above: idempotent handlers, an ack deadline you
extend for slow work, and partitioning by the key whose order you actually
care about rather than hoping for global order.

For the "log as the source of truth" idea, the lineage runs from Google's
*MillWheel: Fault-Tolerant Stream Processing at Internet Scale* (VLDB 2013)
and *The Dataflow Model* (VLDB 2015) — the papers behind Apache Beam — which
formalised event time versus processing time, watermarks, and the question
"when is a window's result complete?" You do not need the paper detail in a
design round, but the vocabulary (event time, late data, watermark) is what
separates a senior answer about a streaming pipeline from a junior one.

**The design-round question this chapter answers:** *"The consumer is down
for an hour. Then it comes back. What does the system do?"* The strong
answer: messages are retained and redelivered; the consumer processes them
idempotently; backpressure is visible as a growing backlog metric with an
alert on age, not size; and if the consumer cannot catch up, you scale it
by partition count, which is why the partition key was chosen up front.
