...
Spanner (OSDI 2012) is again the system to cite, because it is the rare one
that offers true cross-shard, cross-datacenter transactions: two-phase commit
layered over Paxos groups, with TrueTime timestamps giving a global serial
order. It is also the system to cite for what that *costs* — a cross-region
read-write transaction pays at least a cross-datacenter round trip, which is
why the same paper spends so much effort on lock-free snapshot reads: most
work should never take the expensive path.

That is the practical framing for this chapter. Two-phase commit is possible;
Google runs it; and it is still the last resort. The mainstream Google
pattern for cross-service work is the one the *Site Reliability
Engineering* book (2016) and Google Cloud's Pub/Sub documentation describe:
messages are delivered **at least once**, so every consumer must be
idempotent, and ordering is only guaranteed within an explicit ordering key.
"Exactly once" at the system level is then built from at-least-once delivery
plus idempotent handlers plus a deduplication key — not from a transactional
message bus.

**The design-round question this chapter answers:** *"This request charges a
card and writes an order. It times out. What happens?"* The answer is an
idempotency key stored with the first attempt's result, returned on retry; a
saga or outbox for the multi-step case, with compensation for the step that
cannot be rolled back; and a clear statement of which step is the *point of
no return*. Mentioning that even Spanner keeps most reads off the 2PC path
shows you understand the cost, not just the mechanism.
